...
class FLC : public cSimpleModule
{
  public:
    ...
    int fuzzy_inference(double* inp, int nb_inp, int delta);
  ...
  private:
    ...
    Term* m_functions[MAX_NR_INP+1][MAX_NR_TERMS];
    int** rules;
...
int FLC::fuzzy_inference(double* inp, int nb_inp, int delta)
{
    int i, j;
...
void FLC::handleMessage(cMessage *msg)
{
    ...
    {
        double currentDelay = (double)getParentModule()->getSubmodule("sink")->par("meanDelayHP");
        EV << "Current Delay" << currentDelay << "\n";

        if (currentDelay > 0)
        {
            int W_HP = (int)getParentModule()->getSubmodule("scheduler")->par("HighPriorityWeight");
            EV << "High Priority Weight: " << W_HP << "\n";

            double wantedDelay = (double)getParentModule()->par("delayLimit");
            EV << "Wanted Delay: " << wantedDelay << "\n";
            ...
            diff = scale(0, 62, -30, 30, diff);
            W_HP = scale(0, 62, 0, B, W_HP);
            ev << " Dif scalat = " << diff << "\n";
            ev << " weight scalat = " << W_HP << "\n";

            int delta = 2;
            double inp[1] = {diff};

            double result = fuzzy_inference(inp, 1, delta);
            ev << " result = " << result << "\n";
            result_dep.record(result);

            int res = round(scale((B * -1) / 2.0, B / 2.0, 0, 62, result));
            ev << " Result = " << result << "\nRes= " << res << "\n";

            res_dep.record(res);

            new_W_HP = new_W_HP + res;
            EV << "new_W_HP = " << new_W_HP << "\n";

            if (new_W_HP > B) new_W_HP = B - 1;
            if (new_W_HP < 1) new_W_HP = 1;

            cPar& W_HP_r = getParentModule()->getSubmodule("scheduler")->par("HighPriorityWeight");
            ...
            cMessage *flcMsg = new cMessage("flc_command");
            flcMsg->addPar("deltaW").setLongValue(res);
            flcMsg->addPar("newW").setLongValue(new_W_HP);
            send(flcMsg, "out"); // FLC output to scheduler
